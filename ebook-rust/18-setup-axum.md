...
**Lokasi file:** `src/main.rs` (file ini sudah ada, dibuat otomatis oleh `cargo new`)

Buka file `src/main.rs` dan **ganti seluruh isinya** dengan kode berikut:
...
```
support-desk/
├── Cargo.toml                    ← DIUPDATE dengan semua dependencies
├── Cargo.lock                    ← auto-generated oleh Cargo
├── src/
│   └── main.rs                   ← DIUPDATE dengan server yang jalan
└── target/                       ← auto-generated (hasil kompilasi)
```
...
|------|--------|-----------|
| `Cargo.toml` | ✏️ DIUPDATE | Semua 11 dependencies sudah ditambah |
| `src/main.rs` | ✏️ DIUPDATE | 1 route `/health` dengan handler sederhana |
