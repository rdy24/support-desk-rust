...
Perintah ini membuat folder baru bernama `belajar-rust` dengan struktur awal yang sudah siap pakai.

Kamu akan melihat output seperti:

```
Created binary (application) `belajar-rust` package
```
...
[ILUSTRASI: Diagram pohon folder belajar-rust dengan tanda panah menunjuk ke Cargo.toml (berlabel "Konfigurasi"), folder src (berlabel "Kode kita"), dan main.rs (berlabel "Titik masuk program")]
...
**`[dependencies]`** adalah tempat mendaftarkan library tambahan yang dibutuhkan project. Sekarang masih kosong. Di bab-bab berikutnya, kita akan menambahkan library sesuai kebutuhan proyek. Cukup ditambahkan di bagian ini.
...
**`println!("Halo, Rust!")`**: `println` mencetak teks ke layar diikuti baris baru. Tanda seru `!` di belakangnya menandakan ini adalah **macro**, bukan fungsi biasa. Macro itu seperti "shortcut" yang Rust expand menjadi kode yang lebih panjang saat kompilasi. Untuk sekarang, anggap saja `println!` sebagai perintah print yang sedikit "ajaib".

---

## Jalankan Program: `cargo run`

Di terminal, pastikan kamu berada di dalam folder `belajar-rust`, lalu ketik:
...
```
   Compiling belajar-rust v0.1.0 (/Users/kamu/projects/belajar-rust)
    Finished dev [unoptimized + debuginfo] target(s) in 1.23s
     Running `target/debug/belajar-rust`
Halo, Rust!
```

Baris terakhir `Halo, Rust!` adalah output program kita.
...
Sekarang `[dependencies]` di `Cargo.toml` masih kosong. Tapi seiring project berkembang, kita akan menambahkan library-library yang dibutuhkan.
...
Pastikan kamu berhasil menjalankan `cargo run` dan melihat output `Halo, Rust!` di terminal. Kalau belum berhasil, baca ulang bagian instalasi.

**Latihan 2: Ubah Pesan:**
Buka `src/main.rs`, ganti teks `"Halo, Rust!"` dengan nama kamu sendiri, misalnya `"Halo, saya Budi dan saya belajar Rust!"`. Simpan file, lalu jalankan `cargo run` lagi. Pastikan teks baru muncul di layar.

---

Di bab berikutnya, kita bandingkan Rust dengan Go untuk memahami kapan masing-masing bahasa paling cocok dipakai. Sampai jumpa!
